...
function TowerDefense_WaveLoop()
{
	...
	local otherTeam = GetTeamIndex(GetOtherTeams(1 << playerTeam))
...
function TowerDefense_WaveSpawnEnemies( waveNum, waveInfo, otherTeam )
{
	...
	foreach ( spawnInfo in spawngroups )
	{
		...
		if ( !( groupName in level.waveSpawnGroupInfos ) )
			level.waveSpawnGroupInfos[ groupName ] <- { spawnCount = 0, dead = 0, reqDead = 0 }
...
function TryHighPopulationAnnounce()
{
	...
	local nonPlayerTeam = GetTeamIndex(GetOtherTeams(1 << level.nv.attackingTeam))
	...
	while ( 1 )
	{
		...
		foreach ( enemy in enemies )
		{
			if ( IsAlive( enemy ) )
			{
				if ( enemy.IsTitan() )
					aliveTitans++
				else if ( enemy.GetTeam() == GetTeamIndex(GetOtherTeams(1 << level.nv.attackingTeam)))  // flipped spectres shouldn't be counted
					aliveOthers++
			}
		}
...
{
	local otherTeam = GetTeamIndex(GetOtherTeams(1 << level.nv.attackingTeam))
...
{
	local otherTeam = GetTeamIndex(GetOtherTeams(1 << titan.GetTeam()))
...
function GetTitanSpawnPoint( centerOrigin, angles )
{
	...
	if ( !spawnPoint )
		return { origin = centerOrigin, angles = angles }
...
{
	local team 			= GetTeamIndex(GetOtherTeams(1 << level.nv.attackingTeam))
...
function PathFailSolution( npc )
{
	...
	while( 1 )
	{
		...
		if ( killme == true || nearestNode == null || nearestNode < 0 || DistanceSqr( nodePos, npcOrigin ) > STUCK_MAX_NODE_DIST_SQR )
		{
			...
			npc.Die( level.worldspawn, level.worldspawn, { scriptType = DF_INSTANT, damageSourceId = eDamageSourceId.stuck } )
			return
		}
...
{
	local defaultTable = { totalDamage = 0, currentDamageStreak = 0, lastDamageTime = -1, actualDamage = 0 }
...
function Generator_StatusVOThink( generator )
{
	...
	while ( 1 )
	{
		...
		else if ( shieldRatio <= 0.0 && canAnnounceHealthStatus && TimerCheck( "Nag_GeneratorHealth" ) )
		{
			...
			local fracs = {}
			fracs[ 75 ] <- { max = 0.8, min = 0.7 }
			fracs[ 50 ] <- { max = 0.6, min = 0.4 }
			fracs[ 25 ] <- { max = 0.3, min = 0.2 }
...
function GeneratorStatus_PlayersFightingDesperately( shieldRatio, healthRatio )
{
	...
	local playerTeam = level.nv.attackingTeam
	local nonPlayerTeam = GetTeamIndex(GetOtherTeams(1 << playerTeam))
