...
function InitScoreboard()
{
	...
	if (GAMETYPE != FFA) {
		...
		file.teamElems[myTeam].logo.SetImage( "../ui/scoreboard_imc_logo" )
		file.teamElems[enemyTeam].logo.SetImage( "../ui/scoreboard_imc_logo" )
	...
	foreach ( team in teams )
	{
		...
		for ( local elem = 0; elem < level.maxTeamSize; elem++  )
		{
			...
			if( GAMETYPE != COOPERATIVE && GAMETYPE != FFA )
