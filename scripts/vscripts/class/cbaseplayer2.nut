...
CBasePlayer.persistentVars <- null
...
function CBasePlayer::GetPersistentVarDefaultForType( type )
{
...
function CBasePlayer::NormalizePersistentVarValue( type, value )
{
	if ( value == null || value == "pdata_null" )
		return this.GetPersistentVarDefaultForType( type )

	if ( type in pdef_enums )
		return PersistenceEnumValueIsValid( type, value ) ? value : null
	else if ( type == "int" )
...
function CBasePlayer::GetPersistentVar(key) {
	...
	// R1O fake-dedi runs TFO SQ objects, so the native R1 m_ConVars persistent-data
	// bridge is not layout-compatible. Keep per-player session persistence in script
	// memory; missing values use the pdef type defaults just like pdata_null did.
	if ( persistentVars == null )
		persistentVars = {}
	if ( !(key in persistentVars) )
		return this.GetPersistentVarDefaultForType( type )

	return this.NormalizePersistentVarValue( type, persistentVars[key] )
}
...
function CBasePlayer::SetPersistentVar(key, value) {
	...
	local normalizedValue = null
	if ( value == null || value == "" || value == "pdata_null" )
		normalizedValue = this.GetPersistentVarDefaultForType( type )
	else
		normalizedValue = this.NormalizePersistentVarValue( type, value )

	if ( key in persistentVars )
		persistentVars[key] = normalizedValue
	else
		persistentVars[key] <- normalizedValue
}
